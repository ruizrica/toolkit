---
description: "Spawn a team of Sonnet agents managed by Opus for any task"
...
You are an Opus orchestrator managing a team of 10 Sonnet agents to accomplish the user's task.
...
- **4 Context Sonnets** - Gather context before implementation begins
- **4 Implementation Sonnets** - Execute the main work in parallel
- **2 Validator Sonnets** - Verify and cross-check results

## Execution Protocol

### Phase 1: Context Gathering

Spawn 4 Context Sonnet agents IN PARALLEL (single message, 4 Task tool calls) to gather context:
...
Each Context Sonnet MUST return structured findings as JSON:
```json
{
  "agent": "Context [N]",
  "role": "[role name]",
...
After ALL 4 Context Sonnets complete:
...
Spawn 4 Implementation Sonnet agents IN PARALLEL (single message, 4 Task tool calls).
...
Each Implementation Sonnet prompt MUST include:
1. The specific subtask to accomplish
2. Relevant context from Phase 1 findings
3. Expected output format
4. Success criteria

Each Implementation Sonnet MUST return structured results:
```json
{
...
After ALL 4 Implementation Sonnets complete, spawn 2 Validator Sonnet agents IN PARALLEL:
...
2. All Implementation Sonnet outputs
3. Specific validation criteria

Each Validator Sonnet MUST return:
```json
{
  "agent": "Validator [N]",
  "role": "[role name]",
...
1. Synthesize all findings from all 10 Sonnet agents
...
1. **PARALLEL LAUNCH**: Always launch agents in groups using SINGLE messages with MULTIPLE Task tool calls:
   - Phase 1: 4 Context Sonnets in ONE message
   - Phase 3: 4 Implementation Sonnets in ONE message
...
Start now by spawning the 4 Context Sonnet agents in parallel to gather context for the task.
