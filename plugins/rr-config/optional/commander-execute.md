...
- prompt: |
    ...
    **Planning Tasks:**
    ...
    2. **Assign Execution Strategy**
       ...
       - Escalation protocol available (Haiku → Sonnet → Opus)
...
1. **Extract Execution Context from Tasks**
   ...
   ```json
   {
     ...
     "work_type": "backend",
     "execution_agent": "haiku-4.5",
     "escalation_protocol": "haiku-4.5 → sonnet-4.5 → opus-4.5",
     "file_scope": {
       "allowed": ["src/models/user.ts"],
       "forbidden": ["frontend/*"]
     },
...
- Total tasks: X
- Waves: Y
- Execution: Haiku 4.5 (with escalation: Sonnet 4.5 → Opus 4.5)
...
- [123] Implement user model → Haiku 4.5
      Scope: src/models/user.ts, src/types/user.ts
- [124] Create database schema → Haiku 4.5
      Scope: db/migrations/*, db/schema.ts

**Testing Group (parallel)**
- [125] Add unit tests for auth → Haiku 4.5
...
- [126] Create login form → Haiku 4.5
      Scope: frontend/pages/login.tsx
- [127] Add password reset UI → Haiku 4.5
...
- prompt: |
    ...
    **You are Haiku 4.5. If you encounter blockers:**
    ...
    1. **First Escalation (Haiku → Sonnet 4.5):**
    ...
    2. **Second Escalation (Sonnet → Opus 4.5):**
       - Only Sonnet can escalate to Opus
